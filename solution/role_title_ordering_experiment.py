#!/usr/bin/env python3
"""Standalone role-title ordering experiment.

This experiment tests a final bounded ordering term for senior IC roles:
among candidates who are already evidence-qualified, give a small boost when
the current title explicitly matches both the JD's seniority level and role
family. It is intentionally not an eligibility rule and does not use local
proxy labels or generator archetypes.
"""
...
DEFAULT_EXPERIMENT_OUT = DEFAULT_OUTPUT_DIR / "solution_role_title_ordering_experiment.csv"
DEFAULT_EXPERIMENT_REPORT = (
    DEFAULT_OUTPUT_DIR / "solution_role_title_ordering_experiment_summary.json"
)
...
def score_with_role_title_ordering(
    spec: RequirementSpec,
    overlay: dict[str, Any],
    cap: float = DEFAULT_ORDERING_CAP,
) -> tuple[ScoreBreakdown, float]:
    base = score_overlay(spec, overlay, role_title_ordering_cap=0.0)
    if base.blocked:
        return base, 0.0
    modifier = role_title_ordering_modifier(spec, overlay, cap)
    return replace(base, score=base.score + modifier), modifier


def rank_with_role_title_ordering(
    spec: RequirementSpec,
    features_path: Path,
    cap: float = DEFAULT_ORDERING_CAP,
    limit: int = 100,
    ...
    for overlay in iter_jsonl(features_path):
        candidate_count += 1
        row, modifier = score_with_role_title_ordering(spec, overlay, cap)
        if row.blocked:
            blocked_count += 1
            continue
        if modifier > 0:
    ...
    report = {
        "experiment": "role-title-ordering-0.1.0",
...
def main() -> None:
    ...
    rows, report = rank_with_role_title_ordering(
        spec,
    ...
    print(
        f"Role-title ordering experiment ranked {report['candidate_count']:,} candidates "
